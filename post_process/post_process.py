...
class PostProcess:
        ...
        def _evaluate_one(path: str, label: str):
            ...
            if "force_vector" in data and "reference_force_vector" in data:
                ...
                if n > 0:
                    ...
                    rmse_norm = float(
                        np.sqrt(np.mean(np.linalg.norm(err, axis=1) ** 2))
                    )
                    print(f"\n[Force] samples      : {n}")
                    print(
                        f"RMSE Fx,Fy,Fz        : [{rmse_axes[0]:.4f}, {rmse_axes[1]:.4f}, {rmse_axes[2]:.4f}]  [N]"
                    )
            ...
            if "position" in data and "reference_position" in data:
                ...
                if n > 0:
                    ...
                    print(f"\n[Position] samples   : {n}")
                    print(
                        f"RMSE x,y,z           : [{rmse_axes[0]:.4f}, {rmse_axes[1]:.4f}, {rmse_axes[2]:.4f}]  [m]"
                    )
            ...
            if "rotation" in data and "reference_rotation" in data:
                ...
                r_ref = np.asarray(data["reference_rotation"], dtype=float)
                ...
                if n > 0:
                    ...
                    e_r = (Rr.inv() * Rm).as_rotvec()
                    rmse_axes = np.sqrt(np.mean(e_r**2, axis=0))
                    ang = np.linalg.norm(e_r, axis=1)
                    rmse_norm = float(np.sqrt(np.mean(ang**2)))
                    print(f"\n[Rotation] samples   : {n}")
                    print(
                        f"RMSE rotvec x,y,z    : [{rmse_axes[0]:.4f}, {rmse_axes[1]:.4f}, {rmse_axes[2]:.4f}]  [rad]"
                    )
            ...
            if "linear_velocity" in data and "reference_linear_velocity" in data:
                ...
                if n > 0:
                    ...
                    print(
                        f"RMSE vx,vy,vz        : [{rmse_axes[0]:.4f}, {rmse_axes[1]:.4f}, {rmse_axes[2]:.4f}]  [m/s]"
                    )
            ...
            if "angular_velocity" in data and "reference_angular_velocity" in data:
                ...
                if n > 0:
                    ...
                    print(
                        f"RMSE wx,wy,wz        : [{rmse_axes[0]:.4f}, {rmse_axes[1]:.4f}, {rmse_axes[2]:.4f}]  [rad/s]"
                    )
            ...
            Hz = (
                ...
                if "meta__Hz" in data
                    ...
                    data["meta__control_rate_hz"]
                    if "meta__control_rate_hz" in data
                    else None
                )
            )
            if Hz is not None:
        ...
        for arm in ("L", "R"):
            ...
            if kr in data and krref in data:
                ...
                if n > 0:
                    ...
                    rmse_axes = np.sqrt(np.mean(e_r**2, axis=0))
                    ang = np.linalg.norm(e_r, axis=1)
                    rmse_angle = float(np.sqrt(np.mean(ang**2)))
                    print(f"\n[Rotation] samples   : {n}")
                    print(
                        f"RMSE rotvec x,y,z    : [{rmse_axes[0]:.4f}, {rmse_axes[1]:.4f}, {rmse_axes[2]:.4f}]  [rad]"
                    )
    ...
    def evaluate_QP2(
        ...
        if skip_seconds > 0:
            if trim_basis is None:
                print(
                    f"[Trim] Requested skip_seconds={skip_seconds:.3f}s, "
                ...
                if kind == "time":
                    print(
                        f"[Trim] Skipping first {skip_seconds:.3f}s using time vector '{key}' "
                        f"(t0={val:.6f})."
                    )
                elif kind == "dt":
                    print(
                        f"[Trim] Skipping first {skip_seconds:.3f}s using dt '{key}'={val:.6g}s "
                        f"(i0={i0})."
                    )
                elif kind == "Hz":
                    print(
                        f"[Trim] Skipping first {skip_seconds:.3f}s using sample_rate_hz={val:.6g} "
                        f"(i0={i0})."
                    )
        ...
        for arm in ("L", "R"):
            ...
            if kp in data and kpref in data:
                prepped = _prep(
                    np.asarray(data[kp])[..., :3], np.asarray(data[kpref])[..., :3]
                )
                if prepped is not None:
                    A, B, n = prepped
                    err = A - B
            ...
            if kv in data and kvref in data:
                prepped = _prep(
                    np.asarray(data[kv])[..., :3], np.asarray(data[kvref])[..., :3]
                )
                if prepped is not None:
                    A, B, n = prepped
                    err = A - B
            ...
            if kw in data and kwref in data:
                prepped = _prep(
                    np.asarray(data[kw])[..., :3], np.asarray(data[kwref])[..., :3]
                )
                if prepped is not None:
                    A, B, n = prepped
                    err = A - B
    ...
    def plot_PID(self):
        T = (
            np.asarray(self.npz["timestamp"])
            if "timestamp" in self.npz
            else np.array([])
        )
        if T.size == 0:
        ...
        force_p = get2d("force_p_term")
        force_i = get2d("force_i_term")
        force_d = get2d("force_d_term")
        pose_p = get2d("pos_p_term")
        pose_i = get2d("pos_i_term")
        pose_d = get2d("pos_d_term")
        ff_p = get2d("ff_pos_p_term")
        ff_i = get2d("ff_pos_i_term")
        ff_d = get2d("ff_pos_d_term")
        ...
        def fmt(prefix):
            ...
            if kp.size == 1:
                return f"{prefix.upper()} PID: (Kp={kp[0]:.3f}, Ki={ki[0]:.3f}, Kd={kd[0]:.3f})"
            return f"{prefix.upper()} PID: (Kp={kp}, Ki={ki}, Kd={kd})"

        title_force = fmt("force")
        title_pose = fmt("pose")
        title_ff = fmt("ff_pose")
        ...
        block(1, force_p, "Force PID — P")
        block(2, pose_p, "Pose PID — P")
        block(3, ff_p, "FF Pose PID — P")
        block(4, sum_p, "SUM P")
        block(5, force_i, "Force PID — I")
        block(6, pose_i, "Pose PID — I")
        block(7, ff_i, "FF Pose PID — I")
        block(8, sum_i, "SUM I")
        block(9, force_d, "Force PID — D")
        block(10, pose_d, "Pose PID — D")
        block(11, ff_d, "FF Pose PID — D")
        block(12, sum_d, "SUM D")
    ...
    def plot_data3D(self):
        ...
        if ref_pos.shape[-1] == 6:
            ref_pos = ref_pos[:, :3]
        elif ref_pos.size and ref_pos.shape[-1] != 3:
            ref_pos = ref_pos[:, :3]
        ...
        ctrl_dir = (
            np.asarray(self.npz["control_direction"]).reshape(-1)
            if "control_direction" in self.npz
        ...
        f_err_mag = row_norm(f_err)
        p_err_mag = row_norm(p_err)
        f_out_mag = row_norm(f_out)
        p_out_mag = row_norm(p_out)
        ff_po_mag = row_norm(ff_po)
        tot_o_mag = row_norm(tot_o)
        r_err_mag = row_norm(r_err)
        r_out_mag = row_norm(r_out)
        ...
        initial_target_pose_str = (
            f"[{ref_pos[0, 0]:.3f}, {ref_pos[0, 1]:.3f}, {ref_pos[0, 2]:.3f}]"
            if ref_pos.size
            else "[N/A]"
        )
        ...
        def gains_str(base):
            ...
            return (
                f"Kp={kp[0]:.3f}, Ki={ki[0]:.3f}, Kd={kd[0]:.3f}"
                if kp.size == 1
                else f"Kp={kp}, Ki={ki}, Kd={kd}"
            )
        ...
        if ref_forces.size:
            for a, c in enumerate(colors[: min(3, ref_forces.shape[1])]):
                plt.plot(
                    T[: ref_forces.shape[0]],
                    ref_forces[: ref_forces.shape[0], a],
                    "--",
                    label=f"{'XYZ'[a]} Target",
                    lw=2,
                    color=c,
                    alpha=0.7,
                )
        ...
        if has_rot:
            ...
            if rotations.size:
                for a, c in enumerate(colors[: min(3, rotations.shape[1])]):
                    plt.plot(
                        T[: rotations.shape[0]],
                        rotations[: rotations.shape[0], a],
                        label=f"R{'xyz'[a]} Current",
                        lw=2,
                        color=c,
                    )
            if ref_rots.size:
                for a, c in enumerate(colors[: min(3, ref_rots.shape[1])]):
                    plt.plot(
                        T[: ref_rots.shape[0]],
                        ref_rots[: ref_rots.shape[0], a],
                        "--",
                        label=f"R{'xyz'[a]} Target",
                        lw=2,
                        color=c,
                        alpha=0.7,
                    )
        ...
        if "meta__distance_cap" in self.npz:
            ...
            if not np.isnan(cap):
                plt.axhline(
                    y=cap, color="red", ls="--", lw=2, label=f"Distance Cap ({cap}m)"
                )
        ...
        if f_err.size:
            for a, c in enumerate(colors[: min(3, f_err.shape[1])]):
                plt.plot(
                    T[: f_err.shape[0]],
                    f_err[: f_err.shape[0], a],
                    label=f"Force Err {'XYZ'[a]}",
                    lw=2,
                    color=c,
                )
            plt.plot(
        ...
        if p_err.size:
            for a, c in enumerate(colors[: min(3, p_err.shape[1])]):
                plt.plot(
                    T[: p_err.shape[0]],
                    p_err[: p_err.shape[0], a],
                    label=f"Pos Err {'XYZ'[a]}",
                    lw=2,
                    color=c,
                )
            plt.plot(
        ...
        if has_rot:
            ...
            if r_err.size:
                for a, c in enumerate(colors[: min(3, r_err.shape[1])]):
                    plt.plot(
                        T[: r_err.shape[0]],
                        r_err[: r_err.shape[0], a],
                        label=f"Rot Err R{'xyz'[a]}",
                        lw=2,
                        color=c,
                    )
                plt.plot(
        ...
        if f_out.size:
            for a, c in enumerate(colors[: min(3, f_out.shape[1])]):
                plt.plot(
                    T[: f_out.shape[0]],
                    f_out[: f_out.shape[0], a],
                    label=f"Force Out {'XYZ'[a]}",
                    lw=2,
                    color=c,
                )
            plt.plot(
        ...
        if p_out.size:
            for a, c in enumerate(colors[: min(3, p_out.shape[1])]):
                plt.plot(
                    T[: p_out.shape[0]],
                    p_out[: p_out.shape[0], a],
                    label=f"PID Out {'XYZ'[a]}",
                    lw=2,
                    color=c,
                )
        if ff_po.size:
        ...
        plt.axhline(0, color="black", lw=1, alpha=0.5)
        plt.title("3D Position Control Outputs (PID + FF)")
        plt.xlabel("Time (s)")
        ...
        if has_rot:
            ...
            if r_out.size:
                for a, c in enumerate(colors[: min(3, r_out.shape[1])]):
                    plt.plot(
                        T[: r_out.shape[0]],
                        r_out[: r_out.shape[0], a],
                        label=f"Rot Out R{'xyz'[a]}",
                        lw=2,
                        color=c,
                    )
                plt.plot(
        ...
        if tot_o.size:
            for a, c in enumerate(colors[: min(3, tot_o.shape[1])]):
                plt.plot(
                    T[: tot_o.shape[0]],
                    tot_o[: tot_o.shape[0], a],
                    label=f"Total Out {'XYZ'[a]}",
                    lw=2,
                    color=c,
                )
            plt.plot(
    ...
    def plot_box_path_PID(self):
        ...
        if "box_ref_positions" in self.npz and "box_ref_rotmats" in self.npz:
            ...
            "traj_rotation_matrices" in self.npz
            and "frame_R_WB0" in self.npz
            and "frame_p_WB0" in self.npz
        ):
            R_WB0 = np.asarray(self.npz["frame_R_WB0"]).reshape(3, 3)
            p_WB0 = np.asarray(self.npz["frame_p_WB0"]).reshape(
                3,
            )
            rot_updates = np.asarray(self.npz["traj_rotation_matrices"])
            pos_off = (
                np.asarray(self.npz["traj_position_offsets"])
                if "traj_position_offsets" in self.npz
                else None
            )
            K = len(rot_updates)
    ...
    def plot_taskspace(self, arm: str, title_prefix="TaskspaceTracking"):
        ...
        for k in range(3):
            if P.shape[0]:
                ax.plot(
                    t[: P.shape[0]], P[:, k], color=colors[k], label=f"p{labels[k]}"
                )
            if P_ref.shape[0]:
                ax.plot(
                    t[: P_ref.shape[0]],
        ...
        for k in range(3):
            if V.shape[0]:
                ax.plot(
                    t[: V.shape[0]], V[:, k], color=colors[k], label=f"v{labels[k]}"
                )
            if V_ref.shape[0]:
                ax.plot(
                    t[: V_ref.shape[0]],
        ...
        for k in range(3):
            if W.shape[0]:
                ax.plot(
                    t[: W.shape[0]], W[:, k], color=colors[k], label=f"w{labels[k]}"
                )
            if W_ref.shape[0]:
                ax.plot(
                    t[: W_ref.shape[0]],
    ...
    def plot_jointspace(self, arm: str, title_prefix="Joint_Space"):
        ...
        axs[2].legend(fontsize=7, ncol=3)
    ...
    def plot_qp_objective(self, title_prefix="QP_Objective_Breakdown"):
        ...
        ax.plot(
            t[: tot.size], tot, label="Total", linewidth=2.5, color="black", alpha=0.8
        )
        if imp_L.size:
            ax.plot(
                t[: imp_L.size],
    ...
    def plot_qp_performance(self, title_prefix="QP_Performance"):
        ...
        if target_Hz == target_Hz and target_Hz > 0:
            axes[0].axhline(
                ...
                linestyle="--",
                alpha=0.5,
                label=f"{int(target_Hz)}Hz limit",
            )
        ...
        axes[1].plot(
            t[: loop_hz_smooth.size],
            loop_hz_smooth,
            label="Loop Hz (Smoothed)",
            color="blue",
        )
        if target_Hz == target_Hz and target_Hz > 0:
            axes[1].axhline(
                ...
                linestyle="--",
                alpha=0.5,
                label=f"Target {int(target_Hz)}Hz",
            )
    ...
    def plot_force_profile(self, title_prefix="DualAdmittanceAccel2Vel"):
        ...
        ax = axes[0]
        if F_n_L.size:
            ax.plot(t[: F_n_L.size], F_n_L, label="F_n_L")
        if F_n_R.size:
            ax.plot(t[: F_n_R.size], F_n_R, label="F_n_R")
        if F_ref.size:
            ax.plot(t[: F_ref.size], F_ref, "--", label="F_ref")
        ...
        ax = axes[1]
        if x_n_L.size:
            ax.plot(t[: x_n_L.size], x_n_L, label="x_n_L (d_L)")
        if x_n_R.size:
            ax.plot(t[: x_n_R.size], x_n_R, label="x_n_R (d_R)")
        ...
        ax = axes[2]
        if v_n_L.size:
            ax.plot(t[: v_n_L.size], v_n_L, label="v_n_L")
        if v_n_R.size:
            ax.plot(t[: v_n_R.size], v_n_R, label="v_n_R")
                ...
                for k in range(3):
                    ...
                    if g.shape[0]:
                        ax.plot(
                            ...
                            g[:, k],
                            "--",
                            label=f"grasp_{labels_xyz[k]}" if k == 0 else None,
                        )
        ...
        ax = axes2[1, 0]
        if d_L.size:
            ax.plot(t[: d_L.size], d_L, label="d_L (along -n)")
        ax.set_ylabel("d_L [m]")
        ax.grid(True, alpha=0.3)
        ax.legend()
        ...
        ax = axes2[1, 1]
        if d_R.size:
            ax.plot(t[: d_R.size], d_R, label="d_R (along -n)")
        ax.set_ylabel("d_R [m]")
        ax.grid(True, alpha=0.3)
        ax.legend()
        ...
        ax = axes2[2, 0]
        if v_tcp_L_norm.size:
            ax.plot(t[: v_tcp_L_norm.size], v_tcp_L_norm, label="|v_tcp_L| meas")
        if v_des_L_norm.size:
            ax.plot(t[: v_des_L_norm.size], v_des_L_norm, "--", label="|v_des_L| cmd")
        ...
        ax = axes2[2, 1]
        if v_tcp_R_norm.size:
            ax.plot(t[: v_tcp_R_norm.size], v_tcp_R_norm, label="|v_tcp_R| meas")
        if v_des_R_norm.size:
            ax.plot(t[: v_des_R_norm.size], v_des_R_norm, "--", label="|v_des_R| cmd")
    ...
    def plot_box_path_QP(self, title_prefix="Box_Meas_vs_Ref"):
        ...
        R_meas = None
        for side in ("L", "R"):
            pkey = f"box__{side}_positions"
            rkey = f"box__{side}_rotmats"
            if pkey in self.npz and rkey in self.npz:
                pos_meas = np.asarray(self.npz[pkey])
                R_meas = np.asarray(self.npz[rkey])
                break
        ...
        if "box_ref_positions" in self.npz and "box_ref_rotmats" in self.npz:
            ...
            "traj__rotation_matrices" in self.npz
            and "meta__grasping_point_L_world" in self.npz
        ):
            has_ref = False
        ...
        for i, c in enumerate(colors):
            ...
            if has_ref:
                plt.plot(
                    ...
                    color=c,
                    alpha=0.7,
                    label=f"ref {rpy_names[i]}",
                )
        plt.title("Box Orientation vs Time (RPY)")
        plt.xlabel("Time (s)")
        plt.ylabel("Angle (deg)")
        plt.legend()
        plt.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    post_process = PostProcess(
        "experiments/PID_ff/logs/complex/seq_figure8_complex_PID_ff_bw_20251210-150257_L.npz"
    )
