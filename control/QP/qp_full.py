...
from control.PID.pid_controller import flip_xy_component_in_rotation
...
class URImpedanceController(URForceController):
    ...
    def get_M_ur(self, q):
        M_flat = self.rtde_control.getMassMatrix(
            q.tolist(), include_rotors_inertia=True
        )
...
class DualArmImpedanceAdmittanceQP:
    def __init__(
        ...
        self.W_imp_L = (
            np.asarray(W_imp_L, dtype=float) if W_imp_L is not None else np.eye(6)
        )
        self.W_imp_R = (
            np.asarray(W_imp_R, dtype=float) if W_imp_R is not None else np.eye(6)
        )
        self.W_grasp_L_np = (
            np.asarray(W_grasp_L, dtype=float)
            if W_grasp_L is not None
            else np.zeros((6, 6))
        )
        self.W_grasp_R_np = (
            np.asarray(W_grasp_R, dtype=float)
            if W_grasp_R is not None
            else np.zeros((6, 6))
        )
    ...
    def _init_grasping_data(self):
        self.grasping_point_L_world, _, self.normal_L_world = (
            self.robot_L.get_grasping_data()
        )
        self.grasping_point_L_base = self.robot_L.world_point_2_robot(
            self.grasping_point_L_world
        )
        self.normal_L_base = self.robot_L.world_vector_2_robot(self.normal_L_world)

        self.grasping_point_R_world, _, self.normal_R_world = (
            self.robot_R.get_grasping_data()
        )
        self.grasping_point_R_base = self.robot_R.world_point_2_robot(
            self.grasping_point_R_world
        )
        self.normal_R_base = self.robot_R.world_vector_2_robot(self.normal_R_world)

        nL = np.linalg.norm(self.normal_L_base)
        nR = np.linalg.norm(self.normal_R_base)
        if nL > 1e-12:
            self.normal_L_base = self.normal_L_base / nL
        if nR > 1e-12:
            self.normal_R_base = self.normal_R_base / nR
    ...
    def build_qp(self, dt):
        ...
        e_imp_L = (
            self.J_L_p @ self.qddot_L + self.J_dot_L_p @ self.qdot_L_p - self.a_des_L_p
        )
        e_imp_R = (
            self.J_R_p @ self.qddot_R + self.J_dot_R_p @ self.qdot_R_p - self.a_des_R_p
        )
        ...
        # Combined Cost
        obj_L = cp.sum_squares(self.W_imp_L_p @ e_imp_L) + cp.sum_squares(
            self.W_grasp_L_p @ e_grasp_L
        )
        obj_R = cp.sum_squares(self.W_imp_R_p @ e_imp_R) + cp.sum_squares(
            self.W_grasp_R_p @ e_grasp_R
        )
        reg = self.lambda_reg * (
            cp.sum_squares(self.qddot_L) + cp.sum_squares(self.qddot_R)
        )
        obj = obj_L + obj_R + reg

        cons = [
            -self.joint_pose_limit <= q_next_L,
            q_next_L <= self.joint_pose_limit,
            -self.joint_pose_limit <= q_next_R,
            q_next_R <= self.joint_pose_limit,
            -self.joint_speed_limit <= q_dot_next_L,
            q_dot_next_L <= self.joint_speed_limit,
            -self.joint_speed_limit <= q_dot_next_R,
            q_dot_next_R <= self.joint_speed_limit,
            -self.joint_accel_limit <= self.qddot_L,
            self.qddot_L <= self.joint_accel_limit,
            -self.joint_accel_limit <= self.qddot_R,
            self.qddot_R <= self.joint_accel_limit,
        ]
    ...
    def run(self):
        ...
        grasping_R_L_W = RR.from_rotvec(
            self.robot_L.get_state()["pose_world"][3:6]
        ).as_matrix()
        grasping_R_R_W = RR.from_rotvec(
            self.robot_R.get_state()["pose_world"][3:6]
        ).as_matrix()
        ...
        while not self.control_stop.is_set():
            ...
            try:
                ...
                p_L, v_L = (
                    np.array(state_L["gripper_base"][:3]),
                    np.array(state_L["speed"][:3]),
                )
                ...
                p_R, v_R = (
                    np.array(state_R["gripper_base"][:3]),
                    np.array(state_R["speed"][:3]),
                )
                ...
                if elapsed < 5:
                    alpha = elapsed / 5
                    p_ref_L = start_p_L + alpha * (
                        self.grasping_point_L_base - start_p_L
                    )
                    R_ref_L = grasping_R_L
                    v_ref_L, w_ref_L = np.zeros(3), np.zeros(3)

                    p_ref_R = start_p_R + alpha * (
                        self.grasping_point_R_base - start_p_R
                    )
                    ...
                    if not traj_initialized:
                        self._load_refs(
                            self.grasping_point_L_base,
                            self.grasping_point_R_base,
                            grasping_R_L,
                            grasping_R_R,
                            grasping_R_L_W,
                            grasping_R_R_W,
                ...
                f_des_L = self.robot_L.wrench_desired(
                    self.robot_L.K, D_L, e_p_L, e_r_L, e_v_L, e_w_L
                )
                ...
                f_des_R = self.robot_R.wrench_desired(
                    self.robot_R.K, D_R, e_p_R, e_r_R, e_v_R, e_w_R
                )
                ...
                a_n_L = (e_n_L - self.D_a_L * self.v_n_L - self.K_a_L * d_L) / max(
                    self.M_a_L, 1e-9
                )
                a_n_R = (e_n_R - self.D_a_R * self.v_n_R - self.K_a_R * d_R) / max(
                    self.M_a_R, 1e-9
                )
                ...
                imp_L_term = float(
                    (self.W_imp_L @ e_imp_L_val).T @ (self.W_imp_L @ e_imp_L_val)
                )

                x_dot_next_L = J_L @ (qdot_L + qddot_L_cmd * dt)
                e_grasp_L_val = x_dot_next_L - xdot_star_L
                grasp_L_term = float(
                    (self.W_grasp_L_np @ e_grasp_L_val).T
                    @ (self.W_grasp_L_np @ e_grasp_L_val)
                )

                e_imp_R_val = J_R @ qddot_R_cmd + J_dot_R @ qdot_R - a_des_R
                imp_R_term = float(
                    (self.W_imp_R @ e_imp_R_val).T @ (self.W_imp_R @ e_imp_R_val)
                )
                x_dot_next_R = J_R @ (qdot_R + qddot_R_cmd * dt)
                e_grasp_R_val = x_dot_next_R - xdot_star_R
                grasp_R_term = float(
                    (self.W_grasp_R_np @ e_grasp_R_val).T
                    @ (self.W_grasp_R_np @ e_grasp_R_val)
                )

                reg_term = float(self.lambda_reg) * (
                    float(qddot_L_cmd @ qddot_L_cmd) + float(qddot_R_cmd @ qddot_R_cmd)
                )
                obj_total = (
                    imp_L_term + imp_R_term + grasp_L_term + grasp_R_term + reg_term
                )
                ...
                tcp_L = {
                    ...
                    "v": v_L,
                    "w": w_L,
                    "rvec": RR.from_matrix(R_L).as_rotvec(),
                    "p_ref": p_ref_L,
                    "v_ref": v_ref_L,
                    "w_ref": w_ref_L,
                ...
                tcp_R = {
                    ...
                    "v": v_R,
                    "w": w_R,
                    "rvec": RR.from_matrix(R_R).as_rotvec(),
                    "p_ref": p_ref_R,
                    "v_ref": v_ref_R,
                    "w_ref": w_ref_R,
                ...
                self.control_data.append(
                    {
                        ...
                        "obj_break": {
                            "imp_L": imp_L_term,
                            "imp_R": imp_R_term,
                            "grasp_L": grasp_L_term,
                            "grasp_R": grasp_R_term,
                            "reg": reg_term,
                            "total": obj_total,
                        },
                        ...
                        "q_L": q_L,
                        "q_dot_L": qdot_L,
                        "q_ddot_L": qddot_L_cmd,
                        "q_R": q_R,
                        "q_dot_R": qdot_R,
                        "q_ddot_R": qddot_R_cmd,
                        "p_L": p_L,
                        "p_R": p_R,
    ...
    def plot_taskspace(self, arm: str, title_prefix="TaskspaceTracking"):
        ...
        if key not in self.control_data[0]:
            print(f"Key {key} not found in control_data.")
            return
        ...
        fig.suptitle(
            f"{title_prefix} Robot_{arm} – {timestamp_str}", fontsize=14, y=0.99
        )
    ...
    def plot_jointspace(self, arm: str, title_prefix="Joint_Space"):
        ...
        fig.suptitle(
            f"{title_prefix} Robot_{arm} – {timestamp_str}", fontsize=13, y=0.98
        )
    ...
    def plot_qp_performance(self, title_prefix="QP_Performance"):
        ...
        axes[0].axhline(
            y=20.0, color="r", linestyle="--", alpha=0.5, label="20ms limit (50Hz)"
        )
        ...
        axes[1].axhline(
            y=self.Hz, color="k", linestyle="--", alpha=0.5, label=f"Target {self.Hz}Hz"
        )
    ...
    def plot_force_profile(self, title_prefix="DualAdmittanceAccel2Vel"):
        ...
        p_L = np.stack([d["p_L"] for d in data], axis=0)
        ...
        for j in range(2):
            ...
            for k in range(3):
                ...
                ax.plot(
                    t, g[:, k], "--", label=f"grasp_{labels_xyz[k]}" if k == 0 else None
                )
    ...
    def save_everything(self, out_path=None):
        ...
        if cd:
            ...
            if "obj_break" in cd[0]:
                try:
                    pack["obj_imp_L"] = np.array(
                        [d["obj_break"]["imp_L"] for d in cd], dtype=float
                    )
                    pack["obj_imp_R"] = np.array(
                        [d["obj_break"]["imp_R"] for d in cd], dtype=float
                    )
                    pack["obj_grasp_L"] = np.array(
                        [d["obj_break"]["grasp_L"] for d in cd], dtype=float
                    )
                    pack["obj_grasp_R"] = np.array(
                        [d["obj_break"]["grasp_R"] for d in cd], dtype=float
                    )
                    pack["obj_reg"] = np.array(
                        [d["obj_break"]["reg"] for d in cd], dtype=float
                    )
                    pack["obj_total"] = np.array(
                        [d["obj_break"]["total"] for d in cd], dtype=float
                    )
                except Exception:
                    pass
            try:
                pack["status"] = np.array(
                    [str(d.get("status", "")) for d in cd], dtype=object
                )
            except Exception:
                pass
            ...
            try:
                pack["F_L_vec"] = np.array(
                    [d["force_data"]["F_L_vec"] for d in cd], dtype=float
                )
                pack["F_R_vec"] = np.array(
                    [d["force_data"]["F_R_vec"] for d in cd], dtype=float
                )
                pack["F_L_ref"] = np.array(
                    [d["force_data"]["F_L_ref"] for d in cd], dtype=float
                )
                pack["F_R_ref"] = np.array(
                    [d["force_data"]["F_R_ref"] for d in cd], dtype=float
                )
                pack["v_star_L"] = np.array(
                    [d["force_data"]["v_star_L"] for d in cd], dtype=float
                )
                pack["v_star_R"] = np.array(
                    [d["force_data"]["v_star_R"] for d in cd], dtype=float
                )
            except Exception:
                pass
        ...
        for side, robot in (
            ("L", getattr(self, "robot_L", None)),
            ("R", getattr(self, "robot_R", None)),
        ):
            try:
                if robot is not None and hasattr(robot, "box_data") and robot.box_data:
                    ...
                    for item in robot.box_data:
                        if not item:
                            bpos.append([np.nan, np.nan, np.nan])
                            brot.append(np.full((3, 3), np.nan))
                        else:
                            ...
                            bpos.append(
                                np.array(pos, dtype=float).reshape(
                                    3,
                                )
                            )
                            ...
                            if Rm.size == 9:
                                ...
                                Rm = np.full((3, 3), np.nan)
                            brot.append(Rm)
                    pack[f"box__{side}_positions"] = (
                        np.vstack(bpos) if bpos else np.zeros((0, 3))
                    )
                    pack[f"box__{side}_rotmats"] = (
                        np.stack(brot) if brot else np.zeros((0, 3, 3))
                    )
            except Exception:
                pass
        ...
        for name in (
            "grasping_point_L_base",
            "grasping_point_R_base",
            "normal_L_base",
            "normal_R_base",
        ):
            if hasattr(self, name):
        ...
        for name in (
            "grasping_point_L_world",
            "grasping_point_R_world",
            "normal_L_world",
            "normal_R_world",
        ):
            if hasattr(self, name):
        ...
        try:
            pack["meta__planner_npz_path"] = np.array(
                getattr(self, "_npz_path", ""), dtype=object
            )
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        robot_R.go_home()
        robot_L.wait_for_commands()
        robot_R.wait_for_commands()
        robot_L.wait_until_done()
        robot_R.wait_until_done()

        ctrl._init_grasping_data()

        print("Moving to Approach...")
        robot_L.go_to_approach()
        robot_R.go_to_approach()
        robot_L.wait_for_commands()
        robot_R.wait_for_commands()
        robot_L.wait_until_done()
        robot_R.wait_until_done()
