...
class DualArmImpedanceQP:
    ...
    def _build_qp(self, dt):
        ...
        obj = (
            cp.sum_squares(self.W_imp_L_p @ e_imp_L)
        ...
        cons = [
            ...
            -self.joint_accel_limit <= self.q_ddot_L,
            self.q_ddot_L <= self.joint_accel_limit,
            ...
            -self.joint_accel_limit <= self.q_ddot_R,
            self.q_ddot_R <= self.joint_accel_limit,
        ]
    ...
    def run(self):
        ...
        while not self._should_stop.is_set():
            ...
            try:
                ...
                q_ddot_L_sol = (
                    _as_rowvec_1d(self.q_ddot_L.value, "q_ddot_L", 6)
                    if ok
                    else np.zeros(6)
                )
                q_ddot_R_sol = (
                    _as_rowvec_1d(self.q_ddot_R.value, "q_ddot_R", 6)
                    if ok
                    else np.zeros(6)
                )
                ...
                imp_L_term = float(
                    (self.W_imp_L_np @ e_imp_L_val).T @ (self.W_imp_L_np @ e_imp_L_val)
                )
                e_imp_R_val = J_R @ q_ddot_R_sol + J_dot_R @ q_dot_R - a_des_R
                imp_R_term = float(
                    (self.W_imp_R_np @ e_imp_R_val).T @ (self.W_imp_R_np @ e_imp_R_val)
                )
                reg_term = float(self.lambda_reg) * (
                    float(q_ddot_L_sol @ q_ddot_L_sol)
                    + float(q_ddot_R_sol @ q_ddot_R_sol)
                )
                ...
                self.control_data.append(
                    {
                        ...
                        "q_L": q_L,
                        "q_dot_L": q_dot_L,
                        "q_ddot_L": q_ddot_L_sol,
                        "q_R": q_R,
                        "q_dot_R": q_dot_R,
                        "q_ddot_R": q_ddot_R_sol,
                    }
                )
