...
class SingleArmImpedanceQP:
    ...
    def _build_qp(self, dt):
        ...
        obj = cp.sum_squares(self.W_imp_p @ e_imp) + self.lambda_reg * cp.sum_squares(
            self.qdd
        )
    ...
    def log_qp_tick(
        ...
        *,
        ...
        solve_time = _get(
            stats, "solve_time", solve_dt if np.isfinite(solve_dt) else np.nan
        )
        ...
        if not np.isfinite(pri_res):
            pri_res = _get(extra, "pri_res", np.nan)
        dua_res = _get(extra, "dual_residual", np.nan)
        if not np.isfinite(dua_res):
            dua_res = _get(extra, "dua_res", np.nan)

        obj_val = _get(extra, "objective", np.nan)
        if not np.isfinite(obj_val):
            try:
                obj_val = (
                    float(self.prob.value) if self.prob.value is not None else np.nan
                )
            except Exception:
                obj_val = np.nan

        rho_updates = _get(extra, "rho_updates", np.nan)
        if not np.isfinite(rho_updates):
            rho_updates = _get(extra, "n_rho_update", np.nan)
        polish = _get(extra, "polish", None)
        if polish in (None, np.nan):
            polish = _get(extra, "polish_status", None)
        ...
        try:
            ...
            vel_sat = float(
                np.mean(np.abs(qdot_next) >= (self.joint_speed_limit - tol))
            )
        ...
        if M is not None and np.all(np.isfinite(M)):
            try:
                ...
                cond_A = (
                    float(lam_max_A / max(lam_min_A, 1e-12))
                    if np.isfinite(lam_max_A)
                    else np.nan
                )
            except Exception:
        ...
        if (
            ...
            try:
                ...
                tcp = {
                    ...
                    "v": v,
                    "w": w,
                    "rvec": RR.from_matrix(R).as_rotvec(),
                    "p_ref": p_ref,
                    "v_ref": v_ref,
                    "w_ref": w_ref,
        ...
        self.control_data.append(
            {
                ...
                "perf": {
                    "loop_s": t_loop if t_loop is not None else np.nan,
    ...
    def run(self):
        ...
        while not self._should_stop.is_set():
            ...
            try:
                ...
                t_Jdot = time.perf_counter() - t0
                t0 = time.perf_counter()
                M = self.robot.get_M(q)
                t_M = time.perf_counter() - t0
                t0 = time.perf_counter()
                Lam, Lam_inv, lam_info = self.robot.get_Lambda_and_inv(
                    J, M, return_info=True
                )
                t_Lam = time.perf_counter() - t0
                t0 = time.perf_counter()
                ...
                qdd_sol = (
                    _as_rowvec_1d(self.qdd.value, "qdd", length=6)
                    if status_ok and self.qdd.value is not None
                    else np.zeros(6)
                )
                ...
                if (i % 75) == 0:
                    ...
                    lin_ratio = np.linalg.norm(v_pred) / (
                        np.linalg.norm(v_meas) + 1e-12
                    )
                    ang_ratio = np.linalg.norm(w_pred) / (
                        np.linalg.norm(w_meas) + 1e-12
                    )
                    ...
                    print(
                        f"  ‖f_des - f_des_re‖ = {np.linalg.norm(f_des - f_des_re):.6e}"
                    )
                    print(
                        "  (This should be ≈ 0 — otherwise bug in wrench_desired / error stacking.)"
                    )
                    ...
                    print(
                        f"  max |q|      / q_pos_lim  = {np.max(np.abs(q) / self.joint_pose_limit):.3f}"
                    )
                    print(
                        f"  max |q_next| / q_pos_lim  = {np.max(np.abs(q_next) / self.joint_pose_limit):.3f}"
                    )
                    print(
                        f"  max |qdot|   / q_vel_lim  = {np.max(np.abs(qdot) / self.joint_speed_limit):.3f}"
                    )
                    print(
                        f"  max |qdd|    / q_acc_lim  = {np.max(np.abs(qdd_sol) / self.joint_accel_limit):.3f}"
                    )
        ...
        print(
            f"[SingleArmQP SUMMARY] iters={i} | time={total_time:.2f}s | ~{actual_hz:.2f} Hz"
        )
    ...
    def plot_qp_objective(self, title_prefix="QP_Objective_Breakdown"):
        ...
        imp = np.array(
            [d["obj_break"]["imp"] for d in self.control_data if "obj_break" in d]
        )
        reg = np.array(
            [d["obj_break"]["reg"] for d in self.control_data if "obj_break" in d]
        )
        tot = np.array(
            [d["obj_break"]["total"] for d in self.control_data if "obj_break" in d]
        )
        t_o = t[mask]
    ...
    def plot_qp_performance(self, title_prefix="QP_Performance"):
        ...
        model_s = np.array(
            [_get(d, "model_time", "model_s") for d in self.control_data], dtype=float
        )
        solver_s = np.array(
            [_get(d, "solver_time", "solver_s") for d in self.control_data], dtype=float
        )
        loop_s = np.array(
            [_get(d, None, "loop_s") for d in self.control_data], dtype=float
        )

        model_ms, solver_ms, loop_ms = (
            model_s * 1000.0,
            solver_s * 1000.0,
            loop_s * 1000.0,
        )
        ...
        tmin, tmax = (
            (t[np.where(finite_any)[0][0]], t[np.where(finite_any)[0][-1]])
            if finite_any.any()
            else (t[0], t[-1])
        )
        ...
        loop_hz_rm, model_hz_rm, solver_hz_rm = [
            rolling_mean(x, win) for x in (loop_hz, model_hz, solver_hz)
        ]

        TJ = (
            np.array([d.get("perf", {}).get("t_J", np.nan) for d in self.control_data])
            * 1000.0
        )
        TJdot = (
            np.array(
                [d.get("perf", {}).get("t_Jdot", np.nan) for d in self.control_data]
            )
            * 1000.0
        )
        TM = (
            np.array([d.get("perf", {}).get("t_M", np.nan) for d in self.control_data])
            * 1000.0
        )
        TLam = (
            np.array(
                [d.get("perf", {}).get("t_Lam", np.nan) for d in self.control_data]
            )
            * 1000.0
        )
        TD = (
            np.array([d.get("perf", {}).get("t_D", np.nan) for d in self.control_data])
            * 1000.0
        )
        ...
        sev = np.array(
            [classify_status(d.get("status", "")) for d in self.control_data],
            dtype=float,
        )
        ...
        gs = gridspec.GridSpec(
            rows, 1, height_ratios=[2.2, 1.8, 1.4] + ([1.2] if rows == 4 else [])
        )
        ...
        if hasattr(self, "Hz") and np.isfinite(self.Hz):
            ax1.axhline(
                ...
                color="k",
                alpha=0.35,
                label=f"Target {self.Hz:.0f} Hz",
            )
        ax1.set_ylabel("Hz")
        ax1.grid(True, alpha=0.3)
        ax1.legend(loc="best", fontsize=8)
        ax1.set_title("Rolling frequency")
        ...
        if hasattr(self, "dt") and np.isfinite(self.dt):
            ax2.axhline(
                self.dt * 1000.0, ls="--", color="k", alpha=0.25, label="Target dt"
            )
        ax2.set_ylabel("ms")
        ax2.grid(True, alpha=0.3)
        ax2.legend(loc="best", fontsize=8)
        ax2.set_title("Per-iteration timings")
        ax2.set_xlim(tmin, tmax)

        ax3.step(
            t, np.nan_to_num(sev, nan=1.5), where="post", linewidth=2, color="black"
        )
        ...
        ax3.set_yticklabels(["GOOD", "WARN", "BAD"])
        ax3.set_ylim(-0.5, 2.5)
        ax3.set_xlabel("time [s]")
        ax3.set_title("QP solve health")
        ...
        if ax4 is not None:
            ...
            for val, lab in zip(parts, labels):
                ax4.barh(
                    ...
                    [val],
                    left=cum,
                    label=f"{lab} {val:.2f} ms",
                )
                cum += val
            ax4.set_xlabel("ms")
            ax4.grid(True, axis="x", alpha=0.3)
            ax4.set_title("Average model-time breakdown")
            ax4.legend(ncol=5, fontsize=8)
    ...
    def plot_wrench_lambda_inv_minus_xddot(
        self, title_prefix="WrenchLambdaInvMinusXddot"
    ):
        if not self.control_data:
            print("No control_data to plot.")
            return
        ...
        try:
            ...
            print(
                "Missing dynamics['a_des'] or dynamics['a_pred'] in control_data; cannot plot."
            )
            return
